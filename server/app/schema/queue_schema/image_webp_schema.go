...
// producer（UploadService.DefaultConvertImage）与 consumer（task.ProcessImageWebp）共用，
// 字段 json tag 保持一致。
type ImageWebpPayload struct {
	...
	Quality     int `json:"quality"`     // 转换质量0-100
	ScaleWidth  int `json:"scaleWidth"`  // 目标宽度
	ScaleHeight int `json:"scaleHeight"` // 目标高度
}
