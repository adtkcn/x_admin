...
// @Param			menu_type	body		string				true	"权限类型: [M=目录, C=菜单, A=按钮]"
// @Param			menu_name	body		string				true	"菜单名称"
// @Param			menu_icon	body		string				false	"菜单图标"
// @Param			menu_sort	body		int					false	"菜单排序"
...
// @Param			is_cache	body		uint8				false	"是否缓存: [0=否, 1=是]"
// @Param			is_show		body		uint8				false	"是否显示: [0=否, 1=是]"
...
func (mh MenuHandler) Sort(c *gin.Context) {
	var sortReq system_schema.SystemAuthMenuSortReq
	if response.IsFail(c, util.VerifyUtil.VerifyBody(c, &sortReq)) {
		return
	}
	err := system_service.MenuService.Sort(sortReq.Ids)
