...
import (
	...
	"x_admin/app/service/generator_service/tpl_utils"
	"x_admin/config"
	"x_admin/core"
...
// DbTables 库表列表
func (genSrv generateService) DbTables(page request.PageReq, dbReq generator_schema.DbTablesReq) (res response.PageResp, e error) {
	// 分页信息
	limit := page.PageSize
	...
	// 数据
	var tbResp []generator_schema.DbTableResp
...
// List 生成列表
func (genSrv generateService) List(page request.PageReq, listReq generator_schema.ListTableReq) (res response.PageResp, e error) {
	// 分页信息
	limit := page.PageSize
	offset := page.PageSize * (page.PageNo - 1)
	...
	// 数据
	var genResp []generator_schema.GenTableResp
...
func (genSrv generateService) Detail(id string) (res generator_schema.GenTableDetailResp, e error) {
	...
	var base generator_schema.GenTableBaseResp
	convert_util.Copy(&base, genTb)
	var gen generator_schema.GenTableGenResp
	convert_util.Copy(&gen, genTb)
	var colResp []generator_schema.GenColumnResp
	convert_util.Copy(&colResp, columns)
	return generator_schema.GenTableDetailResp{
		Base:   base,
		Gen:    gen,
		Column: colResp,
	}, e
...
func (genSrv generateService) ImportTable(tableNames []string) (e error) {
	var dbTbs []generator_schema.DbTableResp
...
// EditTable 编辑表结构
func (genSrv generateService) EditTable(editReq generator_schema.EditTableReq) (e error) {
