...
type ISettingDictTypeService interface {
	All() (res []setting_schema.SettingDictTypeResp, e error)
	List(page request.PageReq, listReq setting_schema.SettingDictTypeListReq) (res response.PageResp, e error)
	Detail(id string) (res setting_schema.SettingDictTypeResp, e error)
	Add(addReq setting_schema.SettingDictTypeAddReq) (e error)
	Edit(editReq setting_schema.SettingDictTypeEditReq) (e error)
	Del(delReq setting_schema.SettingDictTypeDelReq) (e error)
}
...
// All 字典类型所有
func (dtSrv settingDictTypeService) All() (res []setting_schema.SettingDictTypeResp, e error) {
	var dictTypes []setting_model.DictType
	err := dtSrv.db.Order("id desc").Find(&dictTypes).Error
	if e = response.CheckErr(err, "All Find err"); e != nil {
		return
	}
	res = []setting_schema.SettingDictTypeResp{}
	convert_util.Copy(&res, dictTypes)
	return
}

// List 字典类型列表
func (dtSrv settingDictTypeService) List(page request.PageReq, listReq setting_schema.SettingDictTypeListReq) (res response.PageResp, e error) {
	limit := page.PageSize
	...
	dtResp := []setting_schema.SettingDictTypeResp{}
...
// Add 字典类型新增
func (dtSrv settingDictTypeService) Add(addReq setting_schema.SettingDictTypeAddReq) (e error) {
...
// Edit 字典类型编辑
func (dtSrv settingDictTypeService) Edit(editReq setting_schema.SettingDictTypeEditReq) (e error) {
	// 检查字典类型是否存在
	var dt setting_model.DictType
...
// Del 字典类型删除
func (dtSrv settingDictTypeService) Del(delReq setting_schema.SettingDictTypeDelReq) (e error) {
	if len(delReq.Ids) == 0 {
