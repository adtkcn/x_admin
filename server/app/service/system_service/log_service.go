...
// Operate 系统操作日志
func (logSrv systemLogsServer) OperateLog(page request.PageReq, logReq system_schema.SystemLogOperateReq) (res response.PageResp, e error) {
	// 分页信息
	limit := page.PageSize
	...
	// 数据
	var logResp []system_schema.SystemLogOperateResp
...
// Login 系统登录日志
func (logSrv systemLogsServer) LoginLog(page request.PageReq, logReq system_schema.SystemLogLoginReq) (res response.PageResp, e error) {
	// 分页信息
	limit := page.PageSize
	offset := page.PageSize * (page.PageNo - 1)
	// 查询
	...
	// 数据
	var logResp []system_schema.SystemLogLoginResp
	err = logModel.Limit(limit).Offset(offset).Order("id desc").Find(&logResp).Error
