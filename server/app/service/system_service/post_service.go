...
// All 岗位所有
func (service systemAuthPostService) All() (res []system_schema.SystemAuthPostResp, e error) {
	var posts []system_model.SystemAuthPost
	err := service.db.Order("sort desc, id desc").Find(&posts).Error
	if e = response.CheckErr(err, "All Find err"); e != nil {
		return
	}
	res = []system_schema.SystemAuthPostResp{}
	convert_util.Copy(&res, posts)
	return
}

// List 岗位列表
func (service systemAuthPostService) List(page request.PageReq, listReq system_schema.SystemAuthPostListReq) (res response.PageResp, e error) {
	// 分页信息
	limit := page.PageSize
	...
	postResps := []system_schema.SystemAuthPostResp{}
...
// Add 部门新增
func (service systemAuthPostService) Add(addReq system_schema.SystemAuthPostAddReq) (e error) {
...
// Edit 岗位编辑
func (service systemAuthPostService) Edit(editReq system_schema.SystemAuthPostEditReq) (e error) {
	// 检查岗位是否存在
	var post system_model.SystemAuthPost
