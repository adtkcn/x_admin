...
// getEmailConfig 直接从config.Config.Email随机读取一个邮件配置
func getEmailConfig() (config.EmailConfigStruct, error) {
	if len(config.EmailConfig) == 0 {
		return config.EmailConfigStruct{}, fmt.Errorf("没有可用的邮件配置")
	}
	ran := util.ToolsUtil.Random(0, len(config.EmailConfig))
	return config.EmailConfig[ran], nil
}

// SendEmail 发送邮件的统一入口
func SendEmail(opts EmailOptions) error {
	sendEmail, err := getEmailConfig()
	if err != nil {
		return err
	}
	// 创建邮件消息
