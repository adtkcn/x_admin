...
// SendCode 发送短信验证码
func SendCode(phone, scene string) error {
	// 频率限制: 60s内不可重复发送
	limitKey := fmt.Sprintf(smsCodeLimitTpl, phone)
	if util.RedisUtil.Exists(limitKey) > 0 {
	...
	dailyCount := util.RedisUtil.Get(dailyKey)
	if dailyCount != "" {
	...
	util.RedisUtil.Set(codeKey, codeStr, 300)

	// 设置60s频率限制
	util.RedisUtil.Set(limitKey, "1", 60)

	// 增加每日计数
	if dailyCount == "" {
		util.RedisUtil.Set(dailyKey, "1", 86400) // 24h
	} else {
		util.RedisUtil.Incr(dailyKey)
	}
...
func VerifyCode(phone, scene, code string) error {
	codeKey := fmt.Sprintf(smsCodeKeyTpl, scene, phone)
