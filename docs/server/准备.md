---
name: "后端-前期准备"
description: "x_admin 后端开发前的环境准备：Go/MySQL/Redis 安装与项目初始化"
---
...
### 3.运行后端
1. 复制并重命名`x_admin/server/.env.yaml.example`为`x_admin/server/.env.yaml`文件，
2. `.env.yaml`必须配置 `mysql` 和 `redis` 信息。
```bash
cd x_admin/server
# 生成接口文档
swag init
