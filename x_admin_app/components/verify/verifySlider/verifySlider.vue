			...
			<view class="verify-left-bar"
				...
				<view class="verify-move-block" @touchstart="start" @mousedown="start" @touchend="end" @touchmove="move"
					...
					<view v-if="type === '2'" class="verify-sub-block" :style="{
						...
						<img :src="data.blockBackImgBase" alt="" style="width:100%;height:100%;display:block"></img>
					</view>
				</view>
			</view>
		</view>
...
const move = (e) => {
	...
	barArea.boundingClientRect(rect => {
		...
		if (data.status && data.isEnd == false) {
			...
			//拖动后小方块的left值
			data.moveBlockLeft = (move_block_left - ((props.blockSize.width) / 2)) +
				"px"
...
const getPictrue = () => {
	...
	myRequest({
		...
		let res = result.data
		if (res.repCode == "0000") {
			data.backImgBase = 'data:image/png;base64,' + res.repData.originalImageBase64
			// console.log('this.backImgBase',this.backImgBase);
			console.log('this.backImgBase');
			data.blockBackImgBase = 'data:image/png;base64,' + res.repData.jigsawImageBase64
