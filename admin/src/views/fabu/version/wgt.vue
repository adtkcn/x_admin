...
// 发布热更包（一个版本仅一个已发布包，后端发布时自动取消同版本其它包）
const handleRelease = async (row: type_fabu_wgt_resp) => {
    try {
        await feedback.confirm(
            `确定发布热更包「${row.version} (${row.version_code})」？同版本下其它已发布包将被自动取消。`
        )
        await fabuWgtRelease({ id: row.id, released: true })
        feedback.msgSuccess('发布成功')
        getLists()
    } catch (error) {
        console.error(error)
        getLists() // 确认取消或发布失败时回读，保证状态与服务端一致
    }
}
// 取消发布
const handleCancel = async (row: type_fabu_wgt_resp) => {
    try {
        await feedback.confirm(`确定取消发布热更包「${row.version} (${row.version_code})」？`)
        await fabuWgtRelease({ id: row.id, released: false })
