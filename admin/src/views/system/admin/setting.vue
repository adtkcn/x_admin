    ...
    <div class="user-setting">
        ...
        <el-card class="border-none! mb-4" shadow="never">
            ...
            >
                ...
                <el-form-item v-if="needVerify" label="验证码" prop="email_code">
                    <el-input
        ...
        <el-card class="border-none! mb-4" shadow="never">
            ...
            >
                <input
                    ...
                    "
                />
                <el-form-item label="当前密码" prop="curr_password">
                    <el-input
                        v-model.trim="formData.curr_password"
                        type="password"
                        show-password
                        placeholder="修改密码时必填"
...
import { setUserInfo, sendEmailCode, type type_system_admin_update } from '@/api/system/user'
...
const pwdRules = {
    ...
    password: [
        {
            validator: (_r: any, v: string, cb: any) => {
                if (formData.curr_password && !v) return cb(new Error('请输入新密码'))
                cb()
            },
            trigger: 'blur'
        }
...
async function handleSubmit() {
    try {
        ...
        const info: type_system_admin_update = {
        ...
        if (emailChanged.value) {
            ...
            info.email_code = formData.email_code
        }

        if (formData.password) {
            info.password = encryptPassword(formData.password)
            info.curr_password = encryptPassword(formData.curr_password)
        }
