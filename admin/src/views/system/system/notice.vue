<template>
    <div class="notice">
        <el-card class="border-none!" shadow="never">
            ...
            >
                <vxe-column title="标题" min-width="220">
                    <template #default="{ row }">
                        <!-- 等级圆点前置 + 标题（未读主色标识） -->
                        <div class="flex items-center">
                            <span
                                class="w-[8px] h-[8px] rounded-full shrink-0 mr-2"
                                :style="{ background: getNoticeLevelColor(row.type) }"
                            ></span>
                            <span
                                class="font-bold truncate"
                                :class="{ 'text-primary': row.is_read === 0 }"
                            >
                                {{ row.title }}
                            </span>
                        </div>
                ...
                <vxe-column title="描述" min-width="320">
                    <template #default="{ row }">
                        <div class="text-[13px] text-[var(--el-text-color-secondary)] truncate">
                            {{ row.content }}
                        </div>
                    </template>
                </vxe-column>
...
import { getNoticeLevelColor } from '@/utils/notice'
